...
//-----------------------------------------------------------------------------
// LCE_Patch_Extinction::executeBeforeEachGeneration
//-----------------------------------------------------------------------------
void LCE_Patch_Extinction::executeBeforeEachGeneration(const int& gen)
{
    // temporal parameters
    map<string, Param*>* pParam = _paramSet->getTemporalParams(gen);
    ...
    // set extinction rate from the parameter set
...
bool LCE_Patch_Extinction::init (Metapop* popPtr)
{
    LCE::init(popPtr);
    _Xtion_rate = this->get_parameter_value("extinction_rate");
    if(!_paramSet->isSet("extinction_rate")) {
        _paramSet->set_isSet(false);
        return false;
    }
    return true;
