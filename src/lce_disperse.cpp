...
LCE_Disperse::LCE_Disperse(int rank) : LCE("disperse","",rank),
    ...
    _disp_factor(0), _coln_model(0), _coln_border_model(0),
    _coln_lattice_range(0), _coln_propagule_prob(0), _coln_factor(0)
{
 ...
 * single population. All offsprings are simply transfered to the Disp container
 */
...
void LCE_Disperse::checkDispMatrix (TMatrix* mat, bool coln)
{
    ...
    {
        fatal("The dimension of the dispersal matrix (%ix%i) does not match"
    ...
    for(i = 0; i < _lattice_dims[0]; ++i) {
        ...
        if(abs(1-cntr) > 1e-4){
            warning("The elements of row %i of the dispersal matrix do not sum"
                    "to 1!\n",i+1);
        }
    }
}
...
void LCE_Disperse::execute(){
    ...
    preDispersal();
    (this->*doDispersal)(true);
    // MALCOLM: postColonization();?
...
void LCE_Disperse::postDispersal()
{
    ...
    for(unsigned int i = 0; i < _nb_patch; i++){
        ...
        // MALCOLM TODO: do I need to unmark as extinct?
...
void LCE_Disperse::dispersal_matrix (bool coln)
{
    ...
    for(home = 0; home < _nb_patch; ++home) {
        ...
        for(target=0; target<_nb_patch; ++target){
            // TODO: skip extinct patches if colonizers is set
            ...
            // send the emigrants
            _sendEmigrants(current_patch, home, target, nbInd, migr, factor);
        } // end for each target
...
void LCE_Disperse::_sendEmigrants(Patch* curPatch, const int& home,
        const int& target, int* nbInd, double* migrRate, const double& factor,
        bool coln)
{
    _sendEmigrants(curPatch, home, target, nbInd[FEM], migrRate[FEM], factor,
            FEM);
    _sendEmigrants(curPatch, home, target, nbInd[MAL], migrRate[MAL], factor,
            MAL);
}
 ...
 */
void LCE_Disperse::_sendEmigrants(Patch* curPatch, const int& home,
        const int& target, const int& size, const double& migrRate,
...
bool LCE_Disperse::init (Metapop* popPtr)
{
    ...
    // migration is set by a single dispersal rate (or default)
    else {
    ...
    switch (_disp_model){
        ...
        case 3: // 2D stepping stone
            ...
            if(_lattice_range==0){         // 4 neighbours
                doDispersal = &LCE_Disperse::disperse_2D_stepping_stone_4Neighbour;
                ...
                doDispersal = &LCE_Disperse::disperse_2D_stepping_stone_8Neighbour;
