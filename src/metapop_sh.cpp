...
bool MetapopSH::setStatRecorders(const string& t)
{
    ...
    else if(t=="colnAge_p") add_perPatch("Generations since colonization", "colnAge", FLAT, ALL, 0, 0, 0, &MetapopSH::getColnAge);
...
double MetapopSH::getColnAge( unsigned int i ) {
    if ( _sample_pops[i]->get_isExtinct() ) return 0;
    else return (double)(_sample_pops[i]->colnAge + 1);
}
